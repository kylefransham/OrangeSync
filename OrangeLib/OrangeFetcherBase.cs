...
namespace OrangeLib {
    ...
    public abstract class OrangeFetcherBase {
        ...
        public OrangeFetcherBase (string server, string required_fingerprint, string remote_path,
        ...
        public void Start ()
        {
            ...
            OrangeHelpers.DebugInfo ("Fetcher", "[" + TargetFolder + "] Fetching folder: " + RemoteUrl);
            ...
            if (RequiredFingerprint != null) {
                ...
                if (host_fingerprint == null ||
                    ...
                    OrangeHelpers.DebugInfo ("Auth", "Fingerprint doesn't match");
                ...
                warn = false;
                OrangeHelpers.DebugInfo ("Auth", "Fingerprint matches");

            } else {
               OrangeHelpers.DebugInfo ("Auth", "Skipping fingerprint check");
            }
            ...
            this.thread = new Thread (new ThreadStart (delegate {
                if (Fetch ()) {
                    Thread.Sleep (500);
                    OrangeHelpers.DebugInfo ("Fetcher", "Finished");
                    ...
                    Thread.Sleep (500);
                    OrangeHelpers.DebugInfo ("Fetcher", "Failed");
        ...
        {
            string host = RemoteUrl.Host;
            OrangeHelpers.DebugInfo ("Auth", "Fetching host key for " + host);
        ...
        {
            string tmp_file_path = Path.Combine (OrangeConfig.DefaultConfig.TmpPath, "hostkey.tmp");
            ...
            try {
                ...
                OrangeHelpers.DebugInfo ("Fetcher", "Not a valid fingerprint: " + e.Message);
                return null;
            }
        ...
        {
            string ssh_config_path       = Path.Combine (OrangeConfig.DefaultConfig.HomePath, ".ssh");
            ...
            OrangeHelpers.DebugInfo ("Auth", "Accepted host key for " + host);
