...
namespace OrangeLib.Git {
    ...
    public class OrangeGit : Process {

        public static string ExecPath = null;
        public static string Path     = null;


        public OrangeGit (string path, string args) : base ()
        {
            Path = LocateGit ();
        ...
        {
            OrangeHelpers.DebugInfo ("Cmd", "git " + StartInfo.Arguments);

            try {
                base.Start ();

            } catch (Exception e) {
                OrangeHelpers.DebugInfo ("Cmd", "There's a problem running Git: " + e.Message);
                Environment.Exit (-1);
            }
        }
