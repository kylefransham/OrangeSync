...
namespace OrangeLib.Git {
    ...
    public class OrangeRepo : OrangeRepoBase {

        public OrangeRepo (string path) : base (path)
        {
        }
        ...
        public override string [] UnsyncedFilePaths {
            get {
                ...
                OrangeGit git = new OrangeGit (LocalPath, "status --porcelain");
                git.Start ();
        ...
        public override string CurrentRevision {
            get {
                ...
                string rebase_apply_file = OrangeHelpers.CombineMore (LocalPath, ".git", "rebase-apply");
                if (File.Exists (rebase_apply_file))
                    File.Delete (rebase_apply_file);

                OrangeGit git = new OrangeGit (LocalPath, "log -1 --format=%H");
                git.Start ();
                git.WaitForExit ();
        ...
        {
            get {
                OrangeHelpers.DebugInfo ("Git", "[" + Name + "] Checking for remote changes...");
                OrangeGit git = new OrangeGit (LocalPath, "ls-remote \"" + RemoteUrl + "\" master");
        ...
        public override bool HasUnsyncedChanges {
            get {
                string unsynced_file_path = OrangeHelpers.CombineMore (LocalPath,
...
/*        private void CollectGarbage ()
        ...
        private void PrepareDirectories (string path)
        {
            try {
                foreach (string child_path in Directory.GetDirectories (path)) {
                    if (OrangeHelpers.IsSymlink (child_path))
                        continue;
                    ...
                    if (child_path.EndsWith (".git")) {
                        ...
                        if (File.Exists (HEAD_file_path)) {
                            ...
                            OrangeHelpers.DebugInfo ("Git", "[" + Name + "] Renamed " + HEAD_file_path);
                        }
                ...
                OrangeHelpers.DebugInfo ("Git", "Failed preparing directory: " + e.Message);
            }
        }
        ...
        private double CalculateSizes (DirectoryInfo parent)
        {
            ...
            try {
                ...
                OrangeHelpers.DebugInfo ("Local", "Error calculating size: " + e.Message);
                return 0;
            }
