...
%msbuild% /t:Rebuild /p:Configuration=Release /p:Platform="Any CPU" "%~dp0\OrangeShare.sln"

if "%1"=="installer" (
	if exist "%wixBinDir%" (
	  if exist "%~dp0\OrangeShare.msi" del "%~dp0\OrangeShare.msi"
		...
		"%wixBinDir%\candle" "%~dp0\OrangeShare.wxs" -ext WixUIExtension -ext WixUtilExtension
		...
		"%wixBinDir%\light" -ext WixUIExtension -ext WixUtilExtension Orangeshare.wixobj msysgit.wixobj po.wixobj plugins.wixobj -droot="%~dp0\..\.." -dmsysgitpath="%~dp0\..\..\bin\msysgit" -dpodir="%~dp0\..\..\bin\po" -dpluginsdir="%~dp0\..\..\bin\plugins"  -o OrangeShare.msi 
		if exist "%~dp0\OrangeShare.msi" echo OrangeShare.msi created.
	) else (
